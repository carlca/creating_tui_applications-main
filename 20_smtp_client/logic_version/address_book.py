...
class AddressForm(ModalScreen):
   ...
   def __init__(self, title: str = "Add New Record", initial_data: list | None = None, *args, **kwargs):
   ...
   def compose(self) -> ComposeResult:
      ...
      yield Vertical(
         ...
         self.last_name,
         self.email,
         Horizontal(Button("Save", id="save_address", variant="primary"), Button("Cancel", id="cancel_address", variant="error"), id="address_form_buttons"),
      )
   ...
   def on_save(self) -> None:
      self.dismiss([self.first_name.value, self.last_name.value, self.email.value])
...
class AddressBook(ModalScreen):
   ...
   def compose(self) -> ComposeResult:
      ...
      if not self.get_address:
         yield Vertical(
            ...
            Horizontal(
               Button("New Address", id="new_address", variant="primary"), Button("Edit Address", id="edit_address", variant="error"), id="horizontal_buttons"
            ),
            id="address_book_dlg",
         )
      else:
         yield Vertical(
            DataTable(id="address_book"),
            Horizontal(Button("OK", id="get_address", variant="primary"), Button("Cancel", id="cancel_address", variant="error"), id="horizontal_buttons"),
            id="address_book_dlg",
         )
   ...
   def on_edit_address(self) -> None:
      if self.selected_row is not None:
         self.app.push_screen(AddressForm("Edit Address Record", self.selected_row), self.update_table_and_csv)
