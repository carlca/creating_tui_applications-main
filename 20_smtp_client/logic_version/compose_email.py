...
class ComposeEmailScreen(ModalScreen):
   ...
   def compose(self) -> ComposeResult:
      ...
      yield Header()
      yield Vertical(self.to_addresses, self.email_subject, self.email_body, Button("Send", id="send", variant="primary"), id="compose_email_dlg")
      yield Footer()
   ...
   def on_send_email(self) -> None:
      ...
      if not pathlib.Path("smtp.ini").exists():
         self.app.push_screen(WarningScreen("SMTP Server Settings not set. Go back home and configure it!"))
         return
   ...
   def send_email_with_password(self, password: str = "") -> None:
      if password:
         utility.send_email_with_password(self.smtp_config, self.to_addresses.value, self.email_subject.value, self.email_body.text, password)
