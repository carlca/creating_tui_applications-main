...
from id3_editor import ID3Editor
...
class MP3Player(App):
   ...
   def compose(self) -> ComposeResult:
      ...
      yield Vertical(
         ...
         Center(
            Horizontal(self.stop_btn, self.prev_btn, self.play_btn, self.next_btn, self.pause_btn, id="player-buttons"),
         ),
      )
      yield Footer()
   ...
   def action_edit(self) -> None:
      """
      Edit the ID3 tags of the selected MP3 file
      """
      if pathlib.Path(self.current_track).exists():
         self.push_screen(ID3Editor(self.current_track), self.update_mp3_table)

   def update_mp3_table(self, message) -> None:
      self.id3 = eyed3.load(self.current_track)
      table = self.query_one(DataTable, DataTable)
      values = data_processor.get_mp3(self.current_track)
