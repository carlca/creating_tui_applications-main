...
class Weather(HorizontalGroup):
   ...
   def update_ui(self, location, weather_data):
      ...
      self.app.query_one(f"#current_temp_{self.postal_code}").update(Text(f"Current Temp: {temp} {temp_abbrev}", style="gold1"))
      ...
      for tag in tags:
         ...
         self.app.query_one(f"#{tag}_temp_{self.postal_code}").update(Text(f"Low: {low} {temp_abbrev} / High: {high} {temp_abbrev}", style="orange3"))
