...
class Calculator(App):
   ...
   def on_update_equation(self, event: Button.Pressed) -> None:
      ...
      if label not in self.operators and label not in self.special_keys:
         if current_equation == "0":
            self.solution = f"{label}"
         else:
            self.solution = f"{current_equation}{label}"
      elif label in self.operators and self.last_button_pressed not in self.operators and label not in self.special_keys:
         self.solution = f"{current_equation} {label} "
   ...
   def on_total(self, event: Button.Pressed) -> None:
      ...
      if parts[-1] in self.operators:
         self.solution = self.solution[:-2]

      self.log.info(f"on_total -> {self.solution = }")
